...
def select_RA_seeds(surface, seeds_filename):
    nseeds = 3
    labels = [0, 1, 2]
    if not os.path.exists(seeds_filename):
        ...
        writevtk(pointspd, seeds_filename)

def mark_filled_holes(m_open, surface):
    """Add scalar array with value = 1 in the points corresponding to the filled holes"""
...
def identify_RA_contours(edges, seeds):
    """Given RA edges (hole contours) and manual seeds, separate and identify the different contours: TV (biggest), SVC and IVC"""
    c0 = pointthreshold(edges, 'RegionId', 0, 0)
    c1 = pointthreshold(edges, 'RegionId', 1, 1)
    c2 = pointthreshold(edges, 'RegionId', 2, 2)

    if c0.GetNumberOfPoints() > c1.GetNumberOfPoints() and c0.GetNumberOfPoints() > c2.GetNumberOfPoints():
        tv_cont = c0
        dist1 = euclideandistance(c1.GetPoint(0), seeds.GetPoint(1))  # distance from 2 other conts to SVC
        dist2 = euclideandistance(c2.GetPoint(0), seeds.GetPoint(1))
        if dist1 < dist2:
            svc_cont = c1
            ivc_cont = c2
        else:
            svc_cont = c2
            ivc_cont = c1
    if c1.GetNumberOfPoints() > c0.GetNumberOfPoints() and c1.GetNumberOfPoints() > c2.GetNumberOfPoints():
        tv_cont = c1
        dist1 = euclideandistance(c0.GetPoint(0), seeds.GetPoint(1))  # distance from 2 other conts to SVC
        dist2 = euclideandistance(c2.GetPoint(0), seeds.GetPoint(1))
        if dist1 < dist2:
            svc_cont = c0
            ivc_cont = c2
            ...
            svc_cont = c2
            ivc_cont = c0
    if c2.GetNumberOfPoints() > c0.GetNumberOfPoints() and c2.GetNumberOfPoints() > c1.GetNumberOfPoints():
        tv_cont = c2
        dist1 = euclideandistance(c0.GetPoint(0), seeds.GetPoint(1))  # distance from 2 other conts to SVC
        dist2 = euclideandistance(c1.GetPoint(0), seeds.GetPoint(1))
        if dist1 < dist2:
            svc_cont = c0
            ivc_cont = c1
        else:
            svc_cont = c1
            ivc_cont = c0
    return tv_cont, svc_cont, ivc_cont


def identify_ordered_RA_contours_in_to_be_flat_mesh(locator_open, tv_cont, svc_cont, ivc_cont):
    """Given to_be_flat_mesh and separated countours get the ordered list of ids corresponding to contours in the to_be_flat mesh"""


    # get ordered contour
    tv_conts = get_ordered_cont_ids_based_on_distance(tv_cont)
    svc_conts = get_ordered_cont_ids_based_on_distance(svc_cont)
    ivc_conts = get_ordered_cont_ids_based_on_distance(ivc_cont)

    # get corresponding ordered contours in the to-be-flat mesh
    tv_cont_ids = np.zeros(tv_conts.size)
    svc_cont_ids = np.zeros(svc_conts.size)
    ivc_cont_ids = np.zeros(ivc_conts.size)
    for i in range(tv_cont_ids.shape[0]):
        p = tv_cont.GetPoint(tv_conts[i])
        tv_cont_ids[i] = int(locator_open.FindClosestPoint(p))
    for i in range(svc_cont_ids.shape[0]):
        p = svc_cont.GetPoint(svc_conts[i])
        svc_cont_ids[i] = int(locator_open.FindClosestPoint(p))
    for i in range(ivc_cont_ids.shape[0]):
        p = ivc_cont.GetPoint(ivc_conts[i])
        ivc_cont_ids[i] = int(locator_open.FindClosestPoint(p))

    return tv_cont_ids, svc_cont_ids, ivc_cont_ids


def project_paths_to_open_mesh(m_open, locator_open, path0, path1, path2):
    """Given to_be_flat mesh, corresponding locator and computed dividing paths,
    project paths to the to_be_flat mesh.
    They will be used to identify boundary/dividing paths intersecting points"""
    s0_ids = np.array([])
    for i in range(path0.GetNumberOfPoints()):  # these paths are already ordered
        p = path0.GetPoint(i)
        p_id = locator_open.FindClosestPoint(p)
        if euclideandistance(m_open.GetPoint(p_id), p) < 0.01:  # if close enough, append
            s0_ids = np.append(s0_ids, p_id)

    s1_ids = np.array([])
    for i in range(path1.GetNumberOfPoints()):
        p = path1.GetPoint(i)
        p_id = locator_open.FindClosestPoint(p)
        if euclideandistance(m_open.GetPoint(p_id), p) < 0.01:
            s1_ids = np.append(s1_ids, p_id)
    ...
    for i in range(path2.GetNumberOfPoints()):
        ...
        p_id = locator_open.FindClosestPoint(p)
        if euclideandistance(m_open.GetPoint(p_id), p) < 0.01:
            s2_ids = np.append(s2_ids, p_id)
    return s0_ids, s1_ids, s2_ids


def create_dividing_paths(surface, id_tv0, id_tv1, id_svc, id_ivc, args):
    path0 = find_create_path(surface, id_tv0, id_svc)
    path1 = find_create_path(surface, id_svc, id_ivc)
    path2 = find_create_path(surface, id_ivc, id_tv1)
    writevtk(path0, os.path.join(args.meshfile[0:len(args.meshfile) - 4] + 'path0.vtk'))
    writevtk(path1, os.path.join(args.meshfile[0:len(args.meshfile) - 4] + 'path1.vtk'))
    writevtk(path2, os.path.join(args.meshfile[0:len(args.meshfile) - 4] + 'path2.vtk'))
    return path0, path1, path2


def find_tv_extremes(contour, seeds, locator):
    """Identify TV extremes, i.e. intersections with paths connecting to SVC and IVC"""
    # find closest point in TV contour to IVC and SVC seeds
    locator_tv = vtk.vtkPointLocator()
    locator_tv.SetDataSet(contour)
    locator_tv.BuildLocator()
    id_0 = locator_tv.FindClosestPoint(seeds.GetPoint(1))
    id_1 = locator_tv.FindClosestPoint(seeds.GetPoint(2))
    # in surface
    id_tv0 = locator.FindClosestPoint(contour.GetPoint(id_0))
    id_tv1 = locator.FindClosestPoint(contour.GetPoint(id_1))
    return id_tv0, id_tv1


def reorder_tv(surface, locator_open, tv_cont_ids, id_tv0, id_tv1, args):
    """order tv_cont_ids to start in TV1 (connection with IVC)"""
    final_tv1 = int(locator_open.FindClosestPoint(surface.GetPoint(id_tv1)))
    final_tv0 = int(locator_open.FindClosestPoint(surface.GetPoint(id_tv0)))
    reordered_tv_cont = np.append(tv_cont_ids[int(np.where(tv_cont_ids == final_tv1)[0]): tv_cont_ids.shape[0]],
                                  tv_cont_ids[0: int(np.where(tv_cont_ids == final_tv1)[0])]).astype(int)

    if args.flip == True:
        aux = np.zeros(reordered_tv_cont.size)
        for i in range(reordered_tv_cont.size):
            aux[reordered_tv_cont.size - 1 - i] = reordered_tv_cont[i]
        # mantain the first point (after the flip is the last one)
        reordered_tv_cont = np.append(aux[aux.size - 1], aux[0:aux.size - 1]).astype(int)
    return reordered_tv_cont


def reorder_ivc(ivc_cont_ids, s2_ids, args):
    """order ivc_cont_ids to start in the last point of s2_ids (intersection of path2 and IVC contour)"""
    ivc0 = s2_ids[s2_ids.shape[0] - 1]
    reordered_ivc_cont = np.append(ivc_cont_ids[int(np.where(ivc_cont_ids == ivc0)[0]): ivc_cont_ids.shape[0]],
                                   ivc_cont_ids[0: int(np.where(ivc_cont_ids == ivc0)[0])]).astype(int)

    if args.flip == False:
        aux = np.zeros(reordered_ivc_cont.size)
        for i in range(reordered_ivc_cont.size):
            aux[reordered_ivc_cont.size - 1 - i] = reordered_ivc_cont[i]
        # mantain the v1l as the first one (after the flip is the last one)
        reordered_ivc_cont = np.append(aux[aux.size - 1], aux[0:aux.size - 1]).astype(int)
    return reordered_ivc_cont


def reorder_svc(svc_cont_ids, s1_ids, args):
    # reorder to start in intersection of path1 and SVC contour
    last_s1 = int(s1_ids[s1_ids.shape[0] - 1])
    reordered_svc_cont = np.append(svc_cont_ids[int(np.where(svc_cont_ids == last_s1)[0]): svc_cont_ids.shape[0]],
                                   svc_cont_ids[0: int(np.where(svc_cont_ids == last_s1)[0])]).astype(int)

    if args.flip == False:
        aux = np.zeros(reordered_svc_cont.size)
        for i in range(reordered_svc_cont.size):
            aux[reordered_svc_cont.size - 1 - i] = reordered_svc_cont[i]
        # mantain the v1l as the first one (after the flip is the last one)
        reordered_svc_cont = np.append(aux[aux.size - 1], aux[0:aux.size - 1]).astype(int)
    return reordered_svc_cont
